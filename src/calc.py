...
def main():
    ...
    任务 1：建模与公式
     ...
     2. 元件可靠度
     假设各元件寿命服从指数分布，则单一元件在任务时间T内的可靠度R_i(T)为：R_i(T) = exp(-λ_effi ✖ T)
     ...
     3. 1oo2 并联可靠度
     对于采用 1oo2（双重冗余）并联结构的子系统（如整流模块 C3/C4，起升上限位 C15/C16，小车编码器 C17/C18），只要其中一个元件正常工作即可算作
     子系统成功，其并联可靠度计算公式为：
     R = 1 - (1 - R_A(T))(1 - R_B(T))
